 ...
 * ```
 */
function diffChanges(oldData, newData, excludeFields = []) {
    const changes = {};
 ...
 * ```
 */
function generateChangesSummary(changes, maxLength = 50) {
    const parts = [];
...
function truncateValue(value, maxLength) {
    const str = typeof value === 'object' ? JSON.stringify(value) : String(value ?? 'null');
...
function generateAuditId() {
    return `audit_${crypto_1.default.randomUUID().replace(/-/g, '').substring(0, 16)}`;
}
